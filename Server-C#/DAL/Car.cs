    ...
    public class Car : I_DAL_Car
    {
        public async Task<List<DTO.Car_Dto>> GetAllAsync()
        {
            try
            {
                ...
                var list = Converters.CarConverters.ToListCarDto(await db.Cars.ToListAsync());
                return list;
            }
            catch (Exception e)
            {
        ...
        public async Task<List<DTO.Car_Dto>> AddAsync(DTO.Car_Dto car)
        {
            ...
            {
                Models.RentalCarsContext db = new Models.RentalCarsContext();
                db.Cars.Add(Converters.CarConverters.ToCarModel(car));
                int s = await db.SaveChangesAsync();

                return Converters.CarConverters.ToListCarDto(await db.Cars.ToListAsync());
            }
            catch (Exception e)
            {
                throw e;
